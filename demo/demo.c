...
#define AUDIOFILE "/tmp/demo/shootingstars.raw"
...
const unsigned char open_hatch_msg[]=OPEN_HATCH_MSG;
const unsigned char close_hatch_msg[]=CLOSE_HATCH_MSG;
const unsigned char select_material_msg[]=SELECT_MATERIAL_MSG;
...
#define RinseMotorLen 6 /* number of entries in Phase table */
#define WashMotorLen 4
unsigned char RinseMotor[] = {0x01,0x02,0x04,0x08,0x04,0x02};
unsigned char WashMotor[] = {0x01,0x02,0x04,0x08};
...
void pausell(){
	while(1){
		...
		if(i == '4'){
			system("killall pqiv");
			system("DISPLAY=:0.0 pqiv -f /tmp/demo/2.jpg &");
			initlcd();
			pthread_mutex_lock(&bus_lock);
			CM3_outport(LEDPort, Bin2LED[4]);
			pthread_mutex_unlock(&bus_lock);
			pthread_mutex_lock(&bus_lock);
			LCDprint("Hatch Opened");
			pthread_mutex_unlock(&bus_lock);
				...
				while(1){
					...
					if(i == '5'){
					system("killall pqiv");
					system("DISPLAY=:0.0 pqiv -f /tmp/demo/9.jpg &");
					initlcd();
					pthread_mutex_lock(&bus_lock);
					CM3_outport(LEDPort, Bin2LED[5]);
					pthread_mutex_unlock(&bus_lock);
					pthread_mutex_lock(&bus_lock);
					LCDprint("Hatch Closed");
					pthread_mutex_unlock(&bus_lock);
	...
	while(1)
	{
		...
		{	
			...
			if(i == '5')
			{
				...
				pthread_mutex_lock(&bus_lock);
				LCDprint(close_hatch_msg);
				pthread_mutex_unlock(&bus_lock);

				system("killall pqiv");
				system("DISPLAY=:0.0 pqiv -f /tmp/demo/3.jpg &");
		...
		if(i == '5')
		{
			...
			if(pid==0){
				...
				system("DISPLAY=:0.0 pqiv -f -s --slideshow-interval=0.5 /tmp/demo/demo.jpg /tmp/demo/slide.jpg &");
				return 0;
			}
			else{
...
int main(int agrv,char* argc[])
{
	int* ptr;
    system("killall pqiv");
	system("DISPLAY=:0.0 pqiv -f /tmp/demo/1.jpg &");
	sleep(2);
	CM3DeviceInit();
	CM3DeviceSpiInit(0);
	restart_label:
			...
			{	
				...
				while (1)
				{
					...
					if(i == '5')
					{
						...
						//------- move on to select type of material btn code
						while (1)
						{
							unsigned char i;
							unsigned char k;
							...
							if(i == '1')
							{
								...
								pthread_mutex_lock(&bus_lock);
								LCDprint("Wool Selected");
								pthread_mutex_unlock(&bus_lock);

								system("killall pqiv");
								system("DISPLAY=:0.0 pqiv -f /tmp/demo/4.jpg &");
								...
								i = ScanKey();
									...
									if(i == '8'){
										...
										pthread_mutex_lock(&bus_lock);
										LCDprint("Cleaning Starts Now");
										pthread_mutex_unlock(&bus_lock);

										system("killall pqiv");
										system("DISPLAY=:0.0 pqiv -f /tmp/demo/5.jpg &");
										...
										initlcd();
										pthread_mutex_lock(&bus_lock);
										LCDprint("Rinse cycle");
										pthread_mutex_unlock(&bus_lock);


										//Rinse Cycle
										for(int x=3;x>0;x--){
											CM3_outport(LEDPort, Bin2LED[x]);
											usleep(10000);
											...
											if(i == '9'){
												system("killall pqiv");
												system("DISPLAY=:0.0 pqiv -f /tmp/demo/7.jpg &");
												initlcd();
												pthread_mutex_lock(&bus_lock);
												CM3_outport(LEDPort, Bin2LED[9]);
												pthread_mutex_unlock(&bus_lock);
												pthread_mutex_lock(&bus_lock);
												LCDprint("Cleaning Paused");
												pthread_mutex_unlock(&bus_lock);
												pausell();
												system("killall pqiv");
												system("DISPLAY=:0.0 pqiv -f /tmp/demo/5.jpg &");
											...
											if(i == '4'){
												system("killall pqiv");
												system("DISPLAY=:0.0 pqiv -f /tmp/demo/8.jpg &");
												initlcd();
												pthread_mutex_lock(&bus_lock);
												CM3_outport(LEDPort, Bin2LED[4]);
												pthread_mutex_unlock(&bus_lock);
												pthread_mutex_lock(&bus_lock);
												LCDprint("Accidenly Opened Hatch");
												pthread_mutex_unlock(&bus_lock);
												accidentll();
												system("killall pqiv");
												system("DISPLAY=:0.0 pqiv -f /tmp/demo/5.jpg &");
											...
											{
												CM3_outport(SMPort, WashMotor[a]); /* output to stepper motor */
												usleep(10000); /* delay */
												a++; /* pointer to phase table */
												if (a >= WashMotorLen) a = 0; /* recycle at end of table */
												
												CM3_outport(SMPort, WashMotor[b]); /* output to stepper motor */
												usleep(10000); /* delay */
												b--; /* pointer to phase table */
												if (b <= 0) b = 4; /* recycle at end of table */
											}
											CM3DeviceDeInit();
										}
										...
										initlcd();
										pthread_mutex_lock(&bus_lock);
										LCDprint("Cleaning Done");
										pthread_mutex_unlock(&bus_lock);
										system("killall pqiv");
										system("DISPLAY=:0.0 pqiv -f /tmp/demo/6.jpg &");
